...
\begin{designnote}
  ...
  margin of the system but also within the margin of counting errors during
  the game (are you sure you did not forget a 1 \VP discovery at some point in
  the game?) Of course, if you're playing an EU championship, a 1 \VP
  difference is all it takes to be champion rather than vice-champion\ldots
...
\item remove twice the amount of ongoing loans (\lignebudgetlong{National
    loans at end} plus the amount of national loan not yet refunded).
\end{modlist}
\bparag Divide this result by the number of turns in the period. In case of
transfer or end of game computation, only count the number of turns actually
played.
\bparag Multiply this average Wealth by the percentage indicated in the table
below. This is the amount of Wealth and Prestige \VPs gain by the country.
...
countries) gain 5 \VPs for each \COL or \TP in the specified areas (and
periods)
\bparag \ENG: \continentIndia in periods \period{IV} to \period{VII};
...
(whatever their side) in \continentAmerica; in this case, each \COL\faceplus
...
\aparag[Territorial gains.] Each country gain \VPs equal to the income value
...
\bparag \FRA: former provinces of \paysBourgogne \textmultiply0 during periods
\period{I} and \period{II}, \textmultiply2 during periods \period{V} to
\period{VII}; \regionItalie \textmultiply2 in periods \period{I} to
\period{III}.
...
\bparag Each country earns {\bf 6 \VPs} for each minor country in {\bf
  \VASSAL, \ANNEXION or \dipAT} and {\bf 3 \VPs} for each other controlled
country, with following modifiers:
\bparag \HIS: \regionItalie \textmultiply2; \paysChevaliers\textmultiply2
; do not count autonomous Habsbourg states.
...
\aparag Each player chooses global orientations for its country by selecting 3
out of 5 possible objectives for each period.
...
player's aid. Note that in case of ambiguity, the long list in the rules is
correct and the table is wrong.
...
each player has must secretly pick 3 of the possible 5 objectives for its
country.
...
\bparag At the end of each period (after the last turn of the period),
objectives are checked, and players earn \VPs according to this. A \VP value
is indicated in the tables for each objectives. If a player choose an
objective and complete it, he earn its \VP value, if it is the prioritary one,
he earn twice this value.
