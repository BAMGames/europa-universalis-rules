...
\aparag[Duration.] Objectives only concern what happens during the
period just ended. For example, a ``per turn'' objective in period
\period{II} can only be fulfilled with actions done during period
\period{II}. Similarly, a ``per province annexed'' objective in period
\period{IV} only scores \VPs for provinces that was annexed during
period \period{IV}.

\begin{todo}
  The English objective of \paysEcosse Vassal are incompatible with the
