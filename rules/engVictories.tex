...
% \item[+10] the \seazoneAiguilles sea zone
...
\bparag Discovery of \granderegionPanama by the West is landing one
...
\begin{exemple}[``\granderegionPanama by the West'']
...
\begin{exemple}[Wealth and Prestige \VPs]
...
\begin{exemple}[Colonial situation \VPs]
...
% Avoid \HIS wanting the knights out of Rhodes to start earning VPs
% for them without doing diplomacy on them first.
% (© MKL pouitage)
...
\begin{exemple}[Objective \VPs]
  ...
  2 \textmultiply 45 + 40 + 20 - 50 = 100 \VPs only.

  If Ireland was the \textbf{main objective}, then the maximum is computed
  before doubling, that is, it would result in 2 \textmultiply 40 = 80
  \VPs (plus the other objectives).
\end{exemple}

\begin{exemple}[Mandatory Objectives]
...
\bparag \CCs{Grand Orient} or \CCs{Indian};
...
\bparag \TradeFLEET in any \STZ of the \CCs{Indian}.
...
\begin{exemple}[Commercial \VPs]
  ...
  \item \stz{Noire W}: \HOL (5), \VEN (3), \TUR (2), \FRA (3)
...
(\blasonsmall{naples}): \provinceAbruzzo, \provinceCampania,
...
\begin{exemple}[Provinces ``taken from'' a country]
...
% JC magic
% #2 is almost always #1 with first letter in lowercase. Should be done
% automatically.
...
\EUobjectives{Angleterre}{angleterre} % ANG
% DAN + move HOL here.
\EUobjectives{France}{france} % FRA
\EUobjectives{Espagne}{espagne} % HIS
\EUobjectives{Hollande}{hollande} % HOL
\EUobjectives{Pologne}{pologne} % POL-(PRU)
\EUobjectives{Prusse}{prusse} % (POL)-PRU
\EUobjectives{Portugal}{portugal} % POR-(SUE)
\EUobjectives{Suede}{suede} % (POR)-SUE
\EUobjectives{Russie}{russie} % RUS
\EUobjectives{Turquie}{turquie} % TUR
\EUobjectives{Venise}{venise} % VEN-(AUS)
\EUobjectives{Autriche}{habsbourg} % (VEN)-AUS
...
\aparag[Territory.]
\bparag 1 time the income value of each non national province.
