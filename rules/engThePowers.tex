...
``accidentally'' making errors while computing incomes, expenses or modifiers,
by putting the wrong number of counters on maps, \ldots
...
player performing the action (unless otherwise specified).
...
(\emph{e.g.} for sieges: assault or undermining), then each alliance, in
...
random order.
\bparag Most of the time, the precise order of resolution is not really
important and the actions may be resolved in any order if the players involved
agree. Especially, when different players are involved it is possible to
resolve the actions simultaneously to save time.
...
clarifications on the original \emph{Europa Universalis} game by Philippe
Thibault.
...
\aparag[Religions] Each country, major or minor, has a \terme{religion}.
...
\aparag[Standings] Several religions are further subdivided into
\terme{standings}. Some actions depend not only on the religion but also on
the precise standing of the country.
...
specified (\emph{e.g.} ``Counter-Reformation'' means ``\CATHCR'').
...
\aparag A cultural group usually contains all countries of one or more
religion. Thus, we may use the same names (or symbols) to depict them. But
...
\terme{Counter-Reformation} (also called \terme{Counter-Reform}).

\bparag Unless specified otherwise, consider Catholic minors as being \CATHCR.
...
\terme{Puritan} (available only for \ANG) ; \terme{Rigorous} (or
\terme{Strictly protestant}) and \terme{Tolerant} (available only for \SUE).
...
only to the Latin group and is always Catholic whatever the religion of \POL.
...
\paysGujerat. Other \ROTW countries are considered as having the \terme{Other}
...
\aparag Beware of the distinction between Islam (cultural group) and Muslim
(religion) in the \ROTW (they do contain the exact same European countries).
\bparag Some \ROTW countries are of Muslim religion but belong to the \ROTW
cultural group.
\bparag Some \ROTW countries are of ``Other'' religion but belong to the Islam
cultural group.
\bparag In short: always check the precise religion and cultural group of
\ROTW minors when needed\ldots
...
duration of approximately 30 to 50 years. The 62 game turns are distributed in
seven periods to simulate the different epochs of the era covered by the game.
...
\begin{exemple}[Period limits Table]
...
\bparag If a country has more named leader than its limit, then no unnamed one
is drawn.
...
\begin{exemple}[Turn limits Table]
...
them all successively, he dies at the beginning of the turn written as its last
...
\bparag These levels are recorded with counters on the technology track (on the
\ROTW map).
...
\begin{exemple}[Blocked by a goal]
...
kind: Land or Sea) may never be at the same or adjacent levels and must always
have at least one free level between them. If a goal should move to the same
level as another, or to the level immediately above, it stops two levels
ahead.
...
\begin{exemple}[Stacking technological counters]
...
\aparag List of Land technologies:\par
...
\aparag List of Naval technologies:\par
\begin{tabular}{|l|cc||c|l|}
  \hline
  Name & Year & Turn & First & Remark\\
  \hline
  ...
  \TGLN & 1492 & 1 & \POR, \SPA & Portuguese Nau, Spanish Galleons.\\
...
\subsection{Revolts}
% RaW: 30
\aparag A revolt is determined randomly, concerning where and when it
occurs. A revolt usually brings a drop in the \STAB level of the victim
country. This country has to crush the revolt as quickly as possible otherwise
taking the risk to witness an extension of that revolt (that can go as far as
overthrowing the country's monarch).

\bparag Revolts are generated by events. The revolt tables indicate the
strength and location of the revolt, according to the period in play. The
revolt tables are located at the start of the Events handbook.

\bparag One table is used to determine the victim country. Another group of
tables (one per country) helps determining the revolting province. Finally a
table gives the strength of the revolt.
...
(line \ERS{Occupation, Pillages, Revolts}).
...
% These are rebellions, not revolts
...
\bparag The presence of named \pays{pirates} \LeaderP increase the risk of
pirate appearing.
...
the constant need of (short term) loans that countries of the epoch endured,
...
\aparag Expenses come in two kinds:
...
citizens\ldots (which rather represents refunding old loans and simultaneously
...
\begin{exemple}[Exchequer test]
  A country has a gross income of 300\Ducats. It has a total expenses of
...
\aparag[Exploration] During the military phase, countries may send stacks in
...
\granderegion{Panama}), the principal coast, where the port is located, must
...
province for all military purposes.
...
\begin{exemple}[Progressive appearance of exotic resources]
...
the case of \pays{portugal}). However, it is not possible to turn an
...
\aparag[Competition] Minor establishment cannot be target of competition, and
always win automatic competition.
...
\begin{todo}
  Turn the names into links to the corresponding chapter/section (Diplo).
\end{todo}
