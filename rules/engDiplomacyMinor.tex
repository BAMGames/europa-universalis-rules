...
Actions and diplomatic expenses have to be written on \lignebudget{Diplomatic
  actions}.
% the player's record sheet in the space allocated to "diplomatic actions".
...
% \aparag When all players have completed the writing down of their intended
% actions on the record sheet, each action is resolved.  Two cases are
% possible to solve an action, and will use different procedures, if the
% target minor country is:
% \bparag neutral or controlled by the player attempting the action;
% \bparag under the diplomatic control of another player.
...
% \aparag[Case of a Neutral Minor, or a Minor already Controlled by the
% Player]
% \bparag The action is aimed at increasing the level of control of the player
% on that minor country. The player rolls one die-roll (modified as indicated
% below), then two dice are rolled by any other player to illustrate the
% resistance of that minor to the control attempt.

%% \bparag If the modified die-roll obtained by the player is strictly
%% superior to the sum of the 2 dice obtained, the player earns a number of
%% points of progression equal to the difference between his die roll and the
%% minor's one. These points are used to make the marker of the minor advance
%% on the diplomatic track.

% \bparag The player earns a number of progression points equal to the
% difference between his (modified) die roll and the minor's one.

%% \bparag The player earns a number of points of progression equal to the
%% result of the difference between the player dice and those rolled for the
%% minor.
% \bparag If the difference is null or negative, it does get any points of
% progression (there is no "negative" progression).

% \bparag If several players attempt an action on the same neutral minor
% country, these actions are resolved together at the same time (each player
% rolls his die-roll and modifies it). The player that obtained the best
% result (i.e. the highest modified result) is selected to proceed
% further. Solve ties by competitive unmodified die-rolls.

% \aparag[Case of a Minor Controlled by Another Player] The action is aimed at
% decreasing the level of control of another player on the minor country. It
% may happen at the same time that the controlling player may try to improve
% his level of control on that same minor.

% \bparag[Case of an action made on a minor already on the track of a player
% while that player also makes an action to increase his own level of
% control.]
% There is no "reaction" investment to be paid by the controlling player
% excepts that the player may decide to immediately raise his level of
% investment and pay the difference. This level of investment is paid for his
% own action and is considered the same for the "reaction".  Both actions are
% then resolved at the same time and the best roll is opposed to a resistance
% of 2d10 for the minor country.

% \bparag[The controlling player did not plan to make any action on that
% minor.] He is then allowed to take a "reaction action" on that minor by
% paying the investment required. This reaction is in addition to the actions
% he is normally entitled for the current turn.
% \begin{enumerate}
% \item If the controlling (i.e. defending) power decides to pay no investment
%   in reaction, the minor country is replaced in \Neutral position and
%   defends itself according to his new \Neutral stance: with 2d10.  The
%   acting power earns a calculated number of "points of progression" equal to
%   the difference between his result and 2d10.
% \item If the modified die-roll of the attacking player is strictly superior
%   to the modified die-roll of the defending player, the former earns a
%   calculated number of "points of progression" equal to the difference of
%   die results.
% \item This difference is used to move the minor country diplomatic marker
%   down the track, on the defending player's line on the diplomatic track.
% \item If the result is null or negative, nothing happens and the defending
%   player keeps the marker where it is.
% \end{enumerate}
%% defence
%%% Muf ? Que faisait ce mot tout seul ici ?
%% Va savoir Charles, avec les copier/coller dans tous les sens...

% \bparag Note that the defending player benefits from a bonus applicable to
% the die roll according to the degree of control that he exerts on the minor.
% This bonus is reminded to the player's attention at the top of the
% Diplomatic track on the map.

% \bparag If several players make an action on a same minor country already
% controlled by another player (including one of them), all their actions are
% resolved at the same time. Each player rolls 1d10 (and modifies it as per
% below). The one that has obtained the best result is "selected" (solve ties
% by competitive unmodified die-rolls) to be opposed to the defence based on
% the roll of the controlling player, or to 2d10 (depending on the situation).

\aparag[Modifiers] %This action does not require a table to read its
% results.
...
% PB: never played that way.
% \bparag When it is required to move a marker back, the cost to enter the
% status boxes (backward) is the same as for advancing into it.
...
% , for a number of points of progression equivalent to the remaining balance.
...
\aparag[Full involvement in wars.]
...
war or peacetime.
% \bparag Vassal provinces contribute to domestic and foreign trade.  [TBD:
% les tables disent le contraire] PB: en harmnir maintenant
...
\bparag[Vassals and Separate Peace]
