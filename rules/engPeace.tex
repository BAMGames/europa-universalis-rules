...
\subsubsection{Mandatory peaces}
...
\bparag Exception: if the level of the proposed peace (see below) is 4 or 5,
then the enemy is forced to accept it (this is basically an unconditional
surrender).
...
\begin{exemple}[Disagreements]
  ...
  \begin{itemize}
  \item If \HOL wants to sign a global peace (because \villeAmsterdam is
    besieged) while \ANG wants to stay at war (because it think situation in
    the \ROTW will become better), \HOL may impose its decision to \ANG and
    sign the peace.
...
sign''). Remember that in case of disagreement, the countries stay at war and
that's all.
...
\begin{exemple}[Continued]
  ...
  \item In the same situation, \HIS may decide that is separate peace is valid
    if and only if the Dutch peace is accepted. Typically if \ANG and \HOL try
    to buy \HIS out of the war by offering it an advantageous peace, \HIS may
    link it to the peace with \HOL in order to avoid leaving \FRA alone
    against two enemies.
  \end{itemize}
\end{exemple}
...
alliance, a Peace of level 0 (White Peace) or Negotiated Peace of level 1 is
permitted in favour of any alliance (even the one with the lowest modified
PD, that is the apparent looser).
\bparag Exception: if a power if forced to proposed a Mandatory peace (because
of low \STAB), it must propose a peace of the same level as the PD.
\bparag If at least one Major, part of an alliance, has its capital (or both
...
to loose anything at peace time, but they may not either have any
gain. Obviously, there may be some promise to be fulfilled at a later
diplomatic phase, but as always promises are not binding.
\bparag There are 4 types of conditions that may be given at peace:
\begin{modlist}
\item[Territorial concessions] The losing alliance give ownership of one
...
\item[Special conditions] Event and other specific rules often create specific
  concessions that may (or must) be used as peace condition for some
...
\bparag The order of the concessions is important in case of disagreement
between players.
...
\bparag However, for the precise choice of the condition, the choice is made
...
\begin{exemple}[Disagreement]
  ...
  Since \HOL has the higher \DIP, he choose how to implement the first
  condition and decides that \HIS is going to pay all the indemnities. Even if
  the first territorial concession is the second condition, it is the first
  territorial one, hence chosen by the winning alliance. \FRA chooses to annex
  a Spanish province. Lastly, the second territorial concession is
  implemented. It is chosen by the losing alliance. Since \HOL already has
  its turn in choosing a condition (for the indemnities) and \HIS did not has
  its, \HIS chooses and decide to give a Dutch \COL to \paysPortugal.
...
\bparag[Diplomatic concessions] One country, neither in \VASSAL nor
...
to the winning alliance.
\bparag[Diplomatic concessions] One country (any status, excepted if blocked
...
conditions.
\bparag[Territorial concessions] The first and third territorial concessions
...
\bparag[Territorial concessions] All provinces are chosen by the winning
alliance.
