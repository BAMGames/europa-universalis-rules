...
\aparag[Increase of Inflation] Roll on die. If it is larger than the threshold
on the counter ($\geq 3$ or $\geq 7$ depending on the side up), move the
Inflation marker on box to the right.
...
\bparag Exception: \TUR before some reform use Inflation as if it exploited
...
\begin{exemple}[Computed and Actual Inflation]
  In all this example, assume that the Inflation value is 25\% for the
  concerned countries.

  \FRA has thus a Minimal Inflation of 25\ducats. If its \RT is 60\ducats,
  then its Computed Inflation is 15\ducats (25\% of 15\ducats) and its Actual
  Inflation is 25\ducats (maximum between the Minimal Inflation of 25\ducats
  and the percentage of 15\ducats).
...
\begin{exemple}[Poor countries]
  ...
  \POL has thus a Minimal Inflation of 13\ducats. If its \RT is 40\ducats, the
  Computed Inflation is 10\ducats and the Actual Inflation is 13\ducats. If
  its \RT is -60\ducats, the Computed Inflation is 15\ducats and so is the
  Actual Inflation.
\end{exemple}
