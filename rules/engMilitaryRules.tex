...
\item[+1] If there is an \ASSAULT counter on the fortress;
  % [TBD] if there has been no \terme{Breach} this turn, but an assault
  % that managed to cause at least 1 loss; % before country modif
\item[+2] If there is a \BREACH counter on the fortress;
  % if a \terme{Breach} was obtained previously this turn;
...
\item[+1/+1] If there is an \ASSAULT counter on the fortress.
  % [TBD] if a previous assault caused at least 1 loss. % before
  %                               % country modif
\end{modlist}
...
\terme{Resistance} for any future \terme{Assault}; however, if there is no
...
is at the end of the turn).  Exception: if a \terme{Monarch} happen to be
