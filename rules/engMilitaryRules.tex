...
has an \terme{impulse} where it can moves its troops and fight. Some matters
...
If needed, a new round begins with the \terme{Wintering} segment.
...
Each alliance, in order of initiative, may declare new \terme{Exceptional
  Levies}, use old ones, and refit \terme{Damaged} ships. Check details below.
...
during the previous round and has a ``Supplied'' counter (see naval movement),
...
Once Supply Attrition has been resolved, remove all ``Supplied'' markers from
...
\begin{modlist}
% massed forces cannot be besieged…
% \item[+2] in case of \terme{massed forces} (the stack contains more than 5\LD
%   and 1\Pasha)
...
and its results (especially \terme{foraging}) are applied and effective for
...
\begin{exemple}[Attrition and Interception]
  ...
  ``used'' by the first battle and no more effective. \terme{Foraging} due to
...
\leader{Maetsuycker} only (Dutch leaders with red symbol);
...
\item Dxx: apply~\ref{table:Attrition ROTW Remainders} with xx\% on the number
  ...
  desert and join the conquistador. Use \pays{natives} counters to denote them
...
counter, but, for example, a Polish \FLEET\Facemoins in period \period{I}
...
enough to blockade the supplied port. Place a ``Supplied'' marker on the
...
to decline the possibility to supply in order to continue moving (for example
...
\begin{exemple}[Naval interception modifiers]
  ...
  \seazoneManche the \bonus{+1} is lost (not the same sea zone) for good (it
...
suffers a malus of \bonus{+2} per \Presidio, \StraitFort or \USURE crossed.

Note that even if they may stop naval stacks going through them, neither
...
\bparag[Atlantic] At the beginning of turn 25 (period \period{IV}), all sea
...
\begin{exemple}[Large natives stacks]
  ...
  represent 3\LD and not 8 (35 = 4 \textmultiply 8 + 3). Losses caused by this
...
both sides may have a positive \terme{size differential} during huge battles).
...
% mbox to prevent lone parenthesis at start of line
...
corresponding to the \terme{size differential} (as indicated by the first
...
\begin{designnote}
  ...
  \terme{Size differential} of +2 is quite rare (it requires an \terme{Army
    size} of 7) and +3 can only happen at sea.
\end{designnote}
...
\begin{exemple}[Applying naval losses]
  ...
  \terme{Destroyed}. The losses suffered are still considered 8 for all
  other purposes.
...
If the retreat is done by a phasing stack that did not roll for attrition yet
...
Retreat on Land is not a move. It may not be intercepted and never causes
attrition.
...
\begin{designnote}
  ...
  \terme{Major Battle} if the loosing side has only 2\LD involved. Also, keep
...
\begin{designnote}
  In Europe, if you don't risk \terme{Major Battle}, you can't kill the enemy
...
The \terme{Bad weather} and \terme{Massed force} modifiers are not recalled in
...
\terme{Redeployments}; they are never considered as voluntarily
\terme{Redeployments} (even if the stack has a LoS and is large enough to
...
\terme{Attrition} (one \terme{Siege Attrition} because it is too small and one
\terme{Movement Attrition} because of the redeployment).
...
suffers from \terme{Siege Attrition}. It is possible to both suffer from
...
If the result is strictly smaller than the length of the stack \LoS, then the
stack must roll for \terme{Siege Attrition}.
...
\begin{playtip}
  ...
  \terme{Breach}). Plan this during your wars.
...
(even if a \terme{Breach} was obtained in a previous round, hasty reparations
...
after a \terme{Breach} is much less dangerous.
...
\begin{exemple}[Variations on assault]
  ...
  \texttd\textetoile. While 2\texttd is smaller than 3, the \terme{Breach}
  also reduce the resistance of the fortress to 1. Hence the fortress
  falls. At the same time, \HIS uses the much less deadly \terme{Breach}
  ...
  holding. After a \terme{Breach}, with the same rolls, the losses would be
  only \texttu+\texttd=1, equal to the cap, and the \texttd of the rout is
...
\terme{Exceptional levies} this turn (this round or previously) may recruit
land troops.
