...
new \terme{Exceptional Levies} and use old ones. Refit \terme{Damaged} ships.
...
against \REVOLT/\REBELLION and \corsaire, declare new \terme{Exceptional
  Levies} and use old ones, refit \terme{Damaged} ships. Check details below.
...
\begin{exemple}
  ...
  \terme{Veteran} saves the Spanish troops!
...
\texttu losses, this results in 0 losses, not \texttu-2=-1\texttd).

\GTtable{lossestables}
...
\item at sea, losses inflicted are capped by twice the size of the stack,
  remove any exceeding loses; if a side was routed, the losses it suffer are
...
\item \terme{Damaged} ships are no more battle effective. They may however be
  ...
  (during the Administrative phase). Tally \terme{Damaged} ships on the
...
\begin{exemple}[Applying naval losses]
  ...
  ``only'' 3\texttd are \terme{Damaged} now. The losses suffered stays at
  8\texttd anyway.
\end{exemple}
...
follow-up are counted as battle losses.
...
the commanding \LeaderG (a \terme{Replacement} leader used during the battle
...
Lastly, in Europe, round total losses suffered by each side to the closest
integer. This rounding is part of the losses suffered during the battle.
...
\begin{exemple}[From an actual gaming session]
  ...
  \leader{Rupert} retreats and roll 3, causing \texttu more. The 4\texttu
...
A Battle is called a \terme{Major Battle} of all of the following conditions
are true:
\begin{modlist}
\item one side won;
...
\begin{designnote}
  The battle has to be decisive (rout and size differential). If far away
  (\ROTW), it also need to include sufficiently many (\ARMY counter) ``real''
  (European) troops. Thus, typically, battles between \HIS and \paysAzteque
  are never \terme{Major Battle} because nobody in Europe cares about killing
  a bunch of natives. On the other hand, battles like Saratoga or the Plains
  of Abraham involved European \ARMY counter on the loosing side. They had
  huge impact on the European politics and population and thus do qualify as
...
Exception: if the opposing side had strictly less than 3\LD at the beginning
of the battle, do not roll for leader casualty.

\begin{designnote}
  If you don't risk \terme{Major Defeat}, you can't kill the enemy leader
...
Countries that have less leader than their minimal will get new ones at the
...
\begin{designnote}
  ...
  seriously. It may mean a disgrace (permanent or temporary) removing the
  leader from commanding position, or a severe wound causing the leader to
  stop the military carrier and maybe become minister or ambassador, or
  retiring in his family castle, \ldots
\end{designnote}
