...
roll for \terme{siege attrition}.

Naval stacks never roll for attrition during the supply segment. Beware that
besieged stacks roll for \textbf{siege} attrition during this segment, which
has a similar procedure as supply attrition but slightly different modifiers.
...
The Line of Supply (LoS) goes from a Source of Supply to the stack.
...
Attrition is obtained by rolling one die, modified as follows, and
...
the stack. Note that a result of 10 or less has no effect.

%\GTtable{discoveriesattrition}
\GTtable{causesattrition}
