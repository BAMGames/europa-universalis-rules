...
\begin{exemple}
  ...
  gives \bonus{+2} to the roll. If it loses the battle, since it already
...
Any stack may attempt an exploration, but the presence of a \LeaderE (at sea)
or a \LeaderC/\LeaderGov (on land) will greatly reduce the risks
encountered. A leader alone may also explore but the presence of troops
greatly remove the risks of death during the exploration.
...
Note that \LeaderG or \LeaderA do not use their \Man during exploration. On
the other hand, if a \LeaderMis is present in the stack (even if not
commanding) its \Man may be used instead of the one of the commanding leader.

\LeaderE may explore coastal province (with or without troops) and stay in the
province afterwards; they may never go further inland. In this case, their
\Man is halved (round down).
...
attrition. Additionally, if there are no troops in the stack (leader alone),
if this percentage is 30\% or higher, consider that a \textetoile was obtained
on the ``Discovery'' column (max. 1 \textetoile per exploration).

The ``Discovery'' column contains either a 'S', a 'F' or a '\undemi'; plus
sometimes a \textetoile.
\begin{itemize}
...
\item '\undemi' are resolved by rolling an unmodified die on the small table
  on the right. Treat results of '1--3' as F\textetoile; '4--5' as F; '6--8'
  as S\textetoile; and '9--10' as S.
\item If there is a '\textetoile' (including 30\% loses or more and no troops)
  then the leader (whose \Man was used) must test his death. Roll 1d10, if
  this is strictly more than the \Man (halved for \LeaderE on land as usual),
  the leader is dead (remove him from the game); otherwise, he survives.
\end{itemize}
...
only if some counters are still present. Otherwise, it is assumed that the
whole expedition has been whipped out and the discoveries it may had are
lost\ldots
...
is not in a zone whose discovery has not been brought back yet.

A stack carrying discoveries may ``meet'' another stack if this happen in a
zone whose discovery has been brought back. In that case, if both stacks are
of the same country (not alliance), they share their discoveries: they may
both use the zone discovered by the other and any of them is able to bring all
these back home. Discoveries are never shared that way with another country.
...
zones without malus in the Atlantic Ocean that are known by at least one
country (including minor) are immediately known by everybody; from now on, any
newly explored sea zone without malus from the Atlantic Ocean is known by
