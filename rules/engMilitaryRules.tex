...
chooses which; if it has no \LoS or is too small to undermine, it may not
fight the \REVOLT/\REBELLION either (\emph{i.e.} it must either assault or
redeploy).
...
besieged to twice the resistance of the fortress (read in Subtable
``Fortresses Resistance'') plus the number of \LD inside (\ARMY\facemoins =
2\LD, \ARMY\faceplus=4\LD). Next, add \texttd if the besieger has been
routed.
