...
\subsection{\Presidios and Strait fortifications}
...
used for natives (\pays{natives}), not for \ROTW countries (who use the
...
whole duration of the battle. In case of tie, no side has \terme{Wind
  Advantage}.
...
Remark: The ``5 more \ND'' step will provide a bonus to \terme{Pursuit}.
...
\begin{exemple}[Battle parameters]
  This is late period \period{II} and yet another war occurs in Italy between
  \FRA and \HIS. In the Milanese plains (\provinceLombardia), 1\ARMY\faceplus
  and 1\LD (total of 5\LD) of \terme{Veteran} Spanish Tercios (technologies
  \TARQ and \TTER) encounter 2\ARMY\faceplus (total of 8\LD) of
  \terme{Veteran} French troops (technology \TARQ) coming from the Alps
  (\provinceSavoia).

  First, Morale and CRT columns are computed. With \TARQ, each side has a
  Morale of 2; being \terme{Veteran} each side has a bonus of \bonus{+1} to
  Morale; having \TTER, \HIS has another \bonus{+1} to Morale. Hence the
  Morale is 3 (2+1) for \FRA and 4 (2+1+1) for \HIS.

  Cross-referencing the \TARQ line with the \TARQ column, both sides find a
  ...
  \FRA may have enough\ldots In period \period{II}, \FRA has 3 Artillery per
  ...
  \FRA is of class \CAIV. But the cavalry bonus for class \CAIV only appears
  in period \period{III}. \FRA has 3 more \LD than \HIS and thus gets a
  \bonus{+1} cavalry bonus.

  \HIS is \TTER, this gives a \bonus{-1} malus to the French Shock.

  Once summed, the total modifiers are:
  \begin{itemize}
  \item French Fire: +2 (leader); \quad French Shock: +1 (cavalry) -1 (\TTER) = 0;
  \item Spanish Fire: +0 (nothing); \quad Spanish Shock: +1 (leader).
  \end{itemize}

  Thus, after factoring the second day modifiers, we obtain for \FRA
  +2/0//+1/-1 and for \HIS 0/+1//-1/0. \FRA has an overall slight advantage
  with the DRMs but \HIS has 1 more Morale.

  Note that if \FRA only had 7\LD in the Battle, that would cancel its cavalry
  bonus and result in modifiers of +2/-1//+1/-2 making its overall modifiers
  on par with the Spanish ones. On the other hand, if this was early period
  \period{III}, \FRA would have a \bonus{+1} structural cavalry bonus (its
  armies were typically fielding more cavalries than the Spanish one) for
  final modifiers of +2/+1//+1/0 giving it a very strong advantage in this
  battle. Don't forget to grab all those \bonus{+1}, in the end they will
  save the day\ldots

  The combat result sheet should now looks like:
  \begin{tabular}{c|c}
    \FRA & \HIS \\
    \hline
    +2/0//+1/-1 & 0/+1//-1/0\\
...
\newcommand{\chMilFireTech}{
  \bparag[Technology.] A Stack with \TMED technology does not roll for Fire. A
  stack with \TREN technology only apply the \textetoile. A stack with \TARQ
  technology only does half (round down to lesser \texttu) the indicated
  losses. A stack with \NGD and no \VGD only does half (round up to larger
  \texttu) the indicated losses.}
...
\aparag[End of first day: Destruction.] Using the computation described
at~\ref{chMilitary:Battle:Loss modifications}, check if one side has suffered
more losses than its size. If so, go to~\ref{chMilitary:Battle:Pursuit} (the
Pursuit may cause extra Morale loss and turn this into a rout and, possibly, a
\terme{Major Battle}). The destroyed stack loses the battle and its opponent
wins it.
...
\begin{exemple}
  ...
  +2/0//+1/-1 with a Morale of 3. \HIS has 5\LD and modifiers of 0/+1//-1/0
  with a Morale of 4. Both roll on columns \textbf{C/B}.

  \begin{minipage}{0.7\linewidth}
    \emph{First day, Fire:}
    \begin{itemize}
    \item \FRA rolls 10+2 = 12, resulting in
      2\texttu\textetoile\textetoile. Losses are divided by 2, rounded down
      (Fire with \TARQ), resulting in 1\textetoile\textetoile.
    \item \HIS rolls 6+0 = 6 resulting in \texttu divided by 2 gives nothing.
    \end{itemize}
  \end{minipage} %
  \hfill %
  \begin{minipage}{0.25\linewidth}
    \begin{tabular}{c|c}
      ...
      \hline
      +2/0//+1/-1 & 0/+1//-1/0\\
      \textcolor{red}{|\textetoile\textetoile} &
    \end{tabular}
  \end{minipage}
  ...
  \begin{minipage}{0.7\linewidth}
    ...
    \begin{itemize}
    \item \FRA rolls 6+0 = 6 resulting in \texttd. Its total is now
  ...
  \begin{minipage}{0.25\linewidth}
    \begin{tabular}{c|c}
      ...
      \hline
      +2/0//+1/-1 & 0/+1//-1/0\\
      \textcolor{red}{\normalfont :}|\textetoile\textetoile
           & \textcolor{red}{.|\textetoile}
    \end{tabular}
  \end{minipage}

  \FRA has lost 1 Morale, less than its initial 3. \HIS has lost 2 Morale,
  ...
  eliminated. See details of the computation in~\ref{chMilitary:Battle:Loss
    modifications}. \FRA has actually suffered only \texttd losses
  (1\texttu-\texttd because \HIS only has 5\LD), less than its initial 8\LD
  hence he's still alive. \HIS has actually suffered 2 losses (1\texttd
  becoming 2 because \FRA has +1 size), less than its initial 5\LD hence he's
  still alive. Since there are still men in both troops the battle goes on.
  ...
  lost 2\textetoile\textetoile and feels like a major defeat is coming\ldots
  Since he has 4 in \Man, he decide to try to flee. 4 \Man + 2 remaining
  Morale, the retreat threshold is 6. \HIS rolls\ldots 7 and fails!

  During the second day, \FRA thus gets a +1/+1 modifier (it basically cancels
  the -1/-1 for second day) and thus actually stays at +2/0. \HIS, as
  computed, is now at -1/0.

  Notice that the artillery and cavalry bonus are not recomputed between each
  day and stay the same than in the beginning of the battle, even if some of
  the troops were lost. The actual losses are only implemented at the end of
  the battle and at this stage we still use the initial forces.

  \begin{minipage}{0.7\linewidth}
    \emph{Second day, Fire:}
    \begin{itemize}
    \item \FRA rolls 3+2 = 5 resulting in \texttu divided by 2 gives
      nothing. Its total is now 1\texttd\textetoile\textetoile.
    \item \HIS rolls 10-1 = 9 resulting in 1\texttu\textetoile divided by 2,
      gives \texttd\textetoile. Its total is now 2\textetoile\textetoile.
    \end{itemize}
  \end{minipage} %
  \hfill %
  \begin{minipage}{0.25\linewidth}
    \begin{tabular}{c|c}
      \FRA & \HIS \\
      \hline
      +2/0//\textcolor{red}{+2/0} & 0/+1//-1/0\\
      {\normalfont :}|\textetoile\textetoile
           & .\hspace{-2.3pt}\textcolor{red}{|}|\textetoile\textcolor{red}{\textetoile}
    \end{tabular}
  \end{minipage}
  ...
  \begin{minipage}{0.7\linewidth}
    ...
    \begin{itemize}
    \item \FRA rolls 7+0 = 7 resulting in 1\textetoile. Its total is now
      2\texttd\textetoile\textetoile\textetoile.
    \item \HIS rolls 6+0 = 6 resulting in \texttd. Its total is now
      2\texttd\textetoile\textetoile.
    \end{itemize}
  \end{minipage} %
  \hfill %
  \begin{minipage}{0.25\linewidth}
    \begin{tabular}{c|c}
      ...
      +2/0//\textcolor{red}{+2/0} & 0/+1//-1/0\\
      \textcolor{red}{\normalfont :}{\normalfont
      :}\hspace{-2.15pt}\textcolor{red}{|}|\textetoile\textetoile\textcolor{red}{\textetoile}
           & \textcolor{red}{\normalfont:}||\textetoile\textetoile
    \end{tabular}
  \end{minipage}
