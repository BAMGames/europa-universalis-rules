...
\bparag \xnameref{chMilitary:Battle:Pursuit}: the winner pursue the loser.
\bparag \xnameref{chMilitary:Battle:Loss modifications}: actual losses are
computed from the battle result.
\bparag \xnameref{chMilitary:Battle:Retreat}: any non-winning stack retreat.
...
results that can have drastic influence, especially because due to the
threshold on some effects (\emph{e.g.} 'Major Battle'), a small change can
...
opponent and find two letters. The first is the \terme{Fire CRT column}, the
second is the \terme{Shock} (or \terme{Boarding}) \terme{CRT column}. The bold
...
\item[0/-2] 'Facing the Ottomans' malus (see~\ref{chTurkey:Facing Ottomans}).
\end{modlist}
...
\item[+1/+1] for a side that has \NGD, if both (i) the battle occurs in
  the \region{Mediterranee}; (ii) the other side has \NWD or \NTD and (iii)
...
a 0/+1 DRM if it contains an \ARMY counter of the indicated country or army
class and at least one of the following condition is true:
\begin{modlist}
\item[\CAIIM] this is during periods \period{I} to \period{IV} and the battle
  occurs in a Plain or Sparse Forest province.
\item[\CAIIIM] this is during periods \period{IV} or \period{V} and the battle
  occurs in a Plain or Dense Forest province.
\item[\CAIV] this is period \period{III}, \period{IV} or \period{V} and the
  battle occurs in a Plain province;
\item[\SUE] this is during periods \period{III} to \period{VI} and the battle
  occurs in a Plain or Northern Forest province.
\item[TUR] before reform M-2, if the battle occur in a Plain or Desert
  province (\terme{Sipahi} cavalry).
\end{modlist}
...
\begin{exemple}[Battle parameters]
  ...
  \FRA and \HIS. In the Milanese plains, 1\ARMY\faceplus and 1\LD (total of
  5\LD) of \terme{Veteran} Spanish Tercios (technologies \TARQ and \TTER)
  encounter 2\ARMY\faceplus (total of 8\LD) of \terme{Veteran} French troops
  coming from the Alps (technology \TARQ).
  ...
  Cross-referencing the \TARQ line with the \TARQ column, both side find a
  ...
  \LeaderG while the Spanish are lead by a 323. This result if +2 to Fire for
  ...
  only 5 Artillery which is not enough to get a bonus. Both side has an \ARMY
  ...
  \begin{itemize}
  \item French Fire: +2 (leader);
  \item French Shock: +1 (cavalry) -1 (\TTER) = 0;
  \item Spanish Fire: +0 (nothing);
  \item Spanish Shock: +1 (leader).
  \end{itemize}
...
% \item[0/-1?//-1?/-1?] a side that has lost more morale than the opponent
%   during the previous rolls of this battle has a \bonus{-1} malus to all its
%   rolls. The side suffering from this malus is determined before each roll is
%   made and may vary during battle.
