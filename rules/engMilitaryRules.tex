...
\subsubsection{Sea}
Sea stack have an unlimited movement capacity. A naval stack that does not
stay in a port always roll for attrition (even if it does not move and simply
sstays at sea). The attrition roll is modified by the distance moved, hence
the farther a naval stack move, the more attrition it suffers (especially for
large stacks).
...
certain dangerous sea zone, or to pick up troops for combined move.
...
Actions are normally announced when the naval stack enter the sea zone where
the action will occur. Announcing a naval action ends the movement of the
stack and attrition is rolled immediately before any interception is declared
and before actually resolving the action.
...
supply land troops. However, in this case it can only supply land stack in the
province of the port/arsenal it is blockading.
\bparag[Supplying a besieged port.] The stack must have entered a non-blocked
friendly port during its movement and enter the besieged port. Moreover, it
must be a stack large enough to blockade the supply port. This will remove the
``blockade'' situation of the port for the siege segment of this round
(only). Note that this is only useful if the port is blockaded and thus
require escaping it\ldots
\bparag[Battle.]
\bparag[Fight \corsaire.]
...
do an action. Note that if the land stack tries to disembark in an unknown
province and fails its exploration, it will be destroyed if the naval stack is
not here anymore\ldots

\aparag[List of passive naval actions.]
\bparag[Exploration.] Exploration is resolved during next step.
