...
\aparag[Determine winner.] If one side has already been declared winner,
directly roll for Pursuit.
...
\bparag In case of tie, no side wins. Go to~\ref{chMilitary:Battle:Loss
  modifications}.
...
\begin{exemple}
  In the previous battle, \FRA has 2 Morale left while \HIS has only 1. Hence,
  \FRA wins the battle and pursues the Spanish. The DRM for Pursuit is here 0
  (\Shock differential is negative, battle occurred in Plain, battle ended
  after Second Shock). \FRA rolls 9 which, in column \textbf{E} gives
