...
only a passive campaign during their previous impulse (including, during the
first round of each turn, stacks that had no impulse yet) can only intercept
with stacks containing at most 1 \FLEET counter.

Phasing stacks may intercept non-phasing stack as their action, this is
resolved as a regular interception (exception: if the non-phasing stack is
blockading, this interception automatically succeed). As with any action,
...
\item
  \begin{itemize}
  \item if the moving stack is victorious, it may either roll for attrition
    and do its action (if it declared any) or continue moving (if it declared
    no action);
  \item if the moving stack is defeated, it must retreat and roll for
    attrition with a distance of the whole movement + the whole retreat (and
    without the \Man of its admiral if it has no moral left, as usual with
    retreats).
  \end{itemize}
\end{enumerate}
...
this case, even if this is resolved as an interception battle, this still
count as the action for the moving stack.
...
forces to not take part in the battle.
...
\begin{modlist}
\item[+1] If the intercepting stack has technology \TTD.
