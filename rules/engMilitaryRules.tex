...
A stack must roll for attrition if at least one of the following case is true:
\begin{itemize}
...
\begin{exemple}
  ...
  would trigger attrition, it is not done before the battle as only the last
  ``leg'' is checked at this point. However, if the stack loses its second
  battle, then it must stop movement and hass moved 8\MP, thus it must resolve
