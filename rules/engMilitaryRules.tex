...
and the correct column in Table~\ref{table:Discoveries and Attrition}.
...
\item[+?] if, at the beginning of its movement, the stack is supplied by a
  ...
  \StraitFort, add the DRM of all the \StraitFort along this path (2 in
...
\item[+1/+2] per \PILLAGE\Facemoins/\Faceplus in each province exited or
  entered during the move (\emph{i.e.} count all the \PILLAGE along the path,
  including in the start and end provinces)
\item[+1/+2] per \REVOLT\Facemoins/\Faceplus (or unfriendly
  \REBELLION\Facemoins/\Faceplus) in each province exited or entered during
  the move (same remark)
\item[+?] for each uncontrolled province of the \ROTW cold area, add the
  number of Snowflakes ``resource'' (+0 to +2 depending on the \Area)
\end{modlist}
...
least one is a \ROTW province, use the Attrition in the \ROTW province.
...
\item[+?] if, at the beginning of its movement, the \LoS of the stack goes
  through one or more \StraitFort, add the DRM of all the \StraitFort along
  this path (2 in Europe, level/2 in the \ROTW)
