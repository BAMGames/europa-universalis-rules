...
against revolts ; exploration ; moving \LeaderG (and \LeaderC) alone.
...
or all stacks $\leq$ 5 \LD + 1 \Pasha without restriction
...
\REVOLT/\REBELLION, fortress (even if besieged), \ldots) is considered
``aggressive'' and cannot occur as part as a passive move. Passive campaign
allow only passive moves while other campaigns allow one or more aggressive
moves.
...
\subsubsection{Sea}
Sea stacks have an unlimited movement capacity. A naval stack that does not
...
Each new sea or port entered during the movement is counted as a ``zone'' for
the distance. Entering ports is usually done at the end of movement, to avoid
certain dangerous sea zones, or to pick up troops for combined move.
...
Actions are normally announced when the naval stack enters the sea zone where
the action will occur. Announcing a naval action ends the movement of the
stack and attrition (for the whole movement) is rolled immediately before any
...
\item[+M] \Man differential between the moving leader and the blockading
  one. Only count if it is positive (\emph{i.e.} if the moving leader has more
  \Man).
...
the Supply attrition roll of any troops inside for the next supply step
...
are friendly (include unknown or unoccupied provinces in the \ROTW), the
...
unknown province and fails its exploration, it will be destroyed if the naval
stack is not here anymore\ldots
...
Noter : après bataille navale, interdiction de débuter une transport naval (ou invasion) au
