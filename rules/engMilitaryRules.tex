...
below). If, for some reason, a blockading naval stack is too small to blockade
the fortress (what did you do wrong?), the blockade is lifted and the stack
stays in the sea.

Next, each alliance, in decreasing order of initiative, resolves all of its
actions in an order of its choice (at random in case of disagreement).
...
either besiege or fight the \REVOLT/\REBELLION. If a stack is in a province
with both an enemy fortress and an enemy \REVOLT/\REBELLION, it cannot both
besiege and fight. It must do one of the two but the controller of the stack
chooses which.

\subsection{Sieges}
A besieging stack may either \terme{Undermine} the fortress or \terme{Assault}
it. In order to undermine, a stack must have at least as many \LD as the level
of the fortress. It is possible to decline the possibility to assault, not to
undermine (that is, a too small stack may decide to do nothing because the
assault is too risky but a large enough stack must do something).

Note that if the stack is too small to undermine, it must already have rolled
for \terme{Siege Attrition} at the beginning of the Siege Segment.
...
\item[-?] terrain malus (see details below), non cumulative:
  \begin{modlist}
...
including a European province in the \ROTW and including \COL of level 6:
...
\begin{designnote}[Ports and terrain]
  ...
  blockading the port does not remove this, the fortress is still considered
  to be in Plain, hence there is no malus.
  ...
  cost and did not extend further.

  Forts are very small fortification, hence the malus for port or terrain is
  smaller.
\end{designnote}
...
\item[\textbf{S}\facemoins] add a \USURE\facemoins counter. If there are now
  ...
  \USURE\faceplus. If there are three or more \USURE, only keep the largest
  two.
...
\item[\textbf{B} or \textbf{WH}] The besieger must decide to either apply the
  ...
  split in different provinces).
\item[\textbf{R}] The fortress surrender. It falls and troops inside it are
  ...
  if a \terme{Monarch} happen to be trapped in a surrendering fortress, it is
  captured.
\end{modlist}

When a fortress falls, first put a control marker of the besieged (of the
country of the commander) on the fortress. This fortress is now controlled by
that country. Next, the new controlled must choose to either man the fortress
again or not. If the fortress is manned again, the besieger must loose 1\LD
and the fortress loses 1 level; otherwise, the fortress loses 2 level. In the
\ROTW, fortresses may be entirely destroyed that way. In Europe (including
European provinces in the \ROTW and \COL of level 6), the minimal level a
fortress may have after this is 1. Fortress may ``fall down'' to level 1 even
if the minimal level on the map is 2, in which case a white level 1 fortress
