...
per level of the fortress, it must roll for \terme{Siege Attrition}. If a land
stack is in a province with an enemy fortress and has no \LoS, it must also
roll for \terme{Siege Attrition}. If, for some reason, a blockading naval
...
fallen, troops inside (if any) are removed (captured or killed) and any leader
inside are given back to the besieged who must immediately place them in an
unbesieged friendly province (exception: monarchs are captured).
...
\item[-2] for the \pays{chevaliers} \corsaire, if there is a non-blockaded
  Christian port in the sea zone;
\item[-2] for a \pays{pirates} \corsaire, if there is a \terme{Pirate Haven}
  in the \STZ;
...
\terme{Damaged} \ND are grouped by naval regions (\region{Mediterranee},
all other European seas (``Atlantic (EU)''), \region{Atlantic},
\region{Indian} and \region{Pacific}). Refitted \ND must be placed
