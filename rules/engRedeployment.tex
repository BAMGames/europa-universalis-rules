...
initiative.
\bparag All \corsaire of \terme{Barbaresque} countries are considered as one
...
\bparag \corsaire of \paysPirates target all \TradeFLEET in the \STZ/\CTZ they
are.
\bparag Note that some seas may belong to several \STZ or \CTZ (especially in
Europe). However, \corsaire of \paysPirates are specifically created in one of
them (\emph{e.g.} \ref{eco:Piracy} creates them in the \CTZ of the country
rolling it).
\bparag \corsaire of \terme{Barbaresque} countries attack all christian
\TradeFLEET in the \STZ/\CTZ they are. \TUR must declare when moving them
which \STZ/\CTZ they'll attack
