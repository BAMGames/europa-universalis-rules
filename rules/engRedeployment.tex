...
\item[-M] \Man of one \corsaire \LeaderA/\LeaderP (count only one attacking
  \LeaderA/\LeaderP per attack). %(\textonehalf for land raids in Europe)
...
\item[-2] if the \corsaire of \pays{chevaliers} is in the attack and there is
  a Christian port on \seazone{Egee} or \seazoneLevantin.
\end{modlist}
...
\begin{exemple}[Trade centres losses]
  In period \period{V}, a \HOL-\ANG-\SPA alliance battles \FRA. The Atlantic
  Centre is in \HOL. \ANG chooses to send a \corsaire in \stz{Amerique} in a
...
current price).
\bparag Income received by a minor country (including \pays{pirates}) is
lost.
...
\begin{exemple}[Extension of \REVOLT]
