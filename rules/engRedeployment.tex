...
provinces.
\bparag This province must belong to the same country in case of \REVOLT.
...
\bparag A besieging must retreat if there is no \USURE\Faceplus and there was
neither Breach nor HW obtained during this turn.
...
territory. In case of equality, controller's choice.
\bparag Retreating stack may not enter provinces with non-retreating enemy
troops.
\bparag Retreating troops may use sea move if there is a large enough naval
stack adjacent to the retreating stack.
\bparag Retreating stacks may not split not pick up more troops. All the stack
retreat at the same place.
\bparag Retreating stack may enter or cross province with friendly troops
...
movement attrition with the usual modifier.
...
\aparag[How to build?]
\bparag To build a \Presidio, a country must either control the province
...
directly. The cost is then the sum of cost for each level.

\aparag[\Presidios in play]
\bparag \Presidios are represented by fortresses of the owning power. Thus,
they do count toward the counter limit of that power.
...
\Presidio, it may either dismantle the \Presidio or use it to replace the
regular fortress of the country.
...
\aparag[Before redeployment] For the income phase, see \ref{chIncomes:Gold
  Production}.
\bparag For the transportation for convoys, see
\ruleref{chIncomes:SpanishConvoys} and \ref{chMilitary:Convoys}.
...
during this segment.
\aparag[Gold transportation]\label{chRedep:Gold Transportation} During the
