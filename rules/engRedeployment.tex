...
\bparag A besieging stack must retreat if it is too small to maintain the
...
troops (troops that have finished their retreat are still ``retreating'' until
the end of the segment and thus do not hamper other retreats).
...
\begin{exemple}[Simple retreat]
  ...
  retreat is mandatory and the bonus of \bonus{-2} to attrition not here.
\end{exemple}  

\begin{exemple}[Double retreat]
  ...
  orderly retreat (and the \bonus{-2} to attrition). Note that it is likely
...
\begin{exemple}[Mandatory retreat]
  ...
  supply and must retreat (with a malus of \bonus{+2}). The closest province
...
\begin{exemple}[Impossible retreat]
...
\begin{exemple}[Naval retreat]
  ...
  causes for attrition, each giving a \bonus{+2} to the roll.
...
\subsectionJ{\Presidios}{\includegraphics[width=12bp]{anchor4.png}~\includegraphics[width=12bp]{anchor5.png}}\label{chRedep:BuildPresidios}
...
\begin{modlist}
  \item retreat to any friendly, unblockaded port (or arsenal) of the
