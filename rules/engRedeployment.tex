...
province/settlement (including income of exploited resources at their current price).
...
\aparag \REVOLT/\REBELLION in minor countries are cause loss of \STAB as
if they were in their diplomatic patron.
...
in the same or adjacent province .
\bparag This province must belong to the same country in case of \REVOLT.
\bparag This province must belong to the region allowed by the even in case of
\REBELLION (the same country if no region is specified).
...
way across the \seazoneMan into \ANG (and reciprocally): \provinceUlster is
...
fortress to level 2 (turn 39 or earlier) or 3 (turn 40 or later).
