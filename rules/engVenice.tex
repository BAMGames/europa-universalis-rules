\sectionJ{\anchorpaysmajeur{Venise}}{\blason{venise}}
...
this lowers the control of \VEN, this causes a war of revolt instead
of the usual disannexion: the \MIN declares war unto \VEN, receive
reinforcements but no basic forces. The \MIN controls all the cities
in its provinces. The forces of the \MIN can deploy anywhere in the
\MIN and will attack any Venetian force in the province before the
first military round. The \MAJ that manages to cause this revolt has a
\CB against \VEN at this turn to help the \MIN ; if it uses it, it
obtains the \MIN in \EG, else the \MIN is now Neutral.
\bparag Note that a war of revolt can end by reestablishing the \MIN in
\ANNEXION of \VEN.

\begin{todo}[[BLP]]
  To rewrite. The policy only allows to place occupations in Italy and
  raise the leader limit.
\end{todo}
