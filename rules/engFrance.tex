\sectionJ{\anchorpaysmajeur{France}}{\blason{france}}\label{chSpecific:France}
...
\bparag French \LeaderMis and Missions are either \terme{Catholic} or
\terme{Protestant}. \terme{Catholic} counters have the usual French
shield while \terme{Protestant} counters have the \paysHuguenots
shield.
\bparag If \FRA is \terme{Catholic}, any of its \terme{Protestant}
counters currently in play are immediately destroyed. Missions go back
to the counters pool and \LeaderMis are permanently removed from the
game.
\bparag Similarly, if \FRA is \terme{Protestant}, any of its
\terme{Catholic} counters currently in play are immediately
destroyed. Missions go back to the counters pool and \LeaderMis are
permanently removed from the game.
\bparag Note that the number of counters, as well as the turns of
availability are very different for each religion.
