...
\aparag[] [BLP] Named mercenaries (\leader{J. Cabot} and \leader{S. Cabot})
are added to the pool of mercenaries during their lifetime. Thus, if no
mercenary \LeaderE is available this turn, they are absent.
...
\aparag \SPA, \POR, \FRA and \ANG receive \LeaderMis and \terme{Missions} that
help them in colonial activities. See the specific rules of each power for the
number of \LeaderMis and \terme{Missions} available.
...
\aparag[Missionaries as leaders.] Each \LeaderMis is a leader with values and
...
\aparag[Conquistadors.] A stack with a \LeaderMis has a bonus of {\bf -1} when
...
% returned to its ``Mission'' face in the province where it is and
...
\bparag Exception: \construction{Kaaba} stays in place at all time and may
even co-exist with an establishment of another country.

\aparag If a mission is destroyed, it is returned to the force pool of its
country and may be used again.
\bparag Exception: \construction{Kongo} is removed from game if destroyed.
...
\subsubsectionJ{Ragusa}{\blason{turquievenise}}
...
\subsubsectionJ{Occupation of the Caribbean}{\blason{caraibes}}
...
\subsubsectionJ{The Manila Galleon}{\blason{chineespagne}}
...
\bparag The \stz{Chine} must contain a \TradeFLEET (any level) of either the
\MAJ or \paysChine.
