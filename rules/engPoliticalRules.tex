...
count \POR or \VEN), it is automatically the \terme{Sole Defender of the
  Catholic faith}.
\bparag Alternatively, if there is only one Catholic \MAJ that is
Counter-Reformation, it is automatically the \terme{Sole Defender of the
  Catholic Faith}.

\aparag Else, if more than one country is Catholic, a Catholic \MAJ becomes
the \terme{Sole Defender of the Catholic Faith} when the conditions below are
fulfilled:
...
(either \SPA (if Counter-Reformation) or the \terme{Sole Defender of the
  Catholic Faith}) declares war on a Protestant country and obtains an
...
\item[-2] for each Catholic player at war against another country (\TUR
  excepted).
\item[-3] if the event \eventref{pI:Reformation} has occurred at least once,
  including this turn.
\item[+5] if \ville{Vienne} or \ville{Roma} is captured by the Turks;
\end{modlist}
\bparag If \ville{Roma} falls to the Turks before the end of period II, an
automatic call to the Crusade is launched by the Pope.

\aparag If successful, consider that \eventref{pII:Crusade} is rolled as a
...
\aparag[] [BLP] Named mercenaries (\leader{J Cabot} and \leader{S Cabot}) are
...
\subsubsectionJ{Ragusa}{\blasonJ{turquievenise}}
...
\subsubsectionJ{Occupation of the Caribbean}{\blasonJ{caraibes}}
...
\subsubsectionJ{The Manila Galleon}{\blasonJ{chineespagne}}
