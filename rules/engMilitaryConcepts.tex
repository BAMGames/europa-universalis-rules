...
\definechapterbackground{Military Concepts}{TitianCarlosV}
\chapter{Military Concepts}\label{chapter:MilitaryConcepts}

\newcommand{\StackArtillery}[1][]{%
% used both in Description of forces and in stacking
% #1: label, only in stacking.
...
% used both in Description of Morale and in Stacking.
% #1: \aparag name
...
\terme{Cavalry} and its \terme{Artillery value}.
...
\aparag[Artillery value.] Each \ARMY\facemoins and \ARMY\faceplus has a
certain \terme{Artillery value}. This value is an abstract representation
(rather than an actual number of guns and howitzers) of the amount and
efficiency of field and siege artillery.
\bparag The \terme{Artillery value} of an \ARMY\faceplus can be read
in~\ref{table:Artillery Per Army} by cross-referencing the country (or class)
with the current period.
\bparag The \terme{Artillery value} of an \ARMY\facemoins is always half the
\terme{Artillery value} of an \ARMY\faceplus (rounded down).

\GTtable{artilleryvalue}

\StackArtillery

\begin{exemple}[Military doctrine]
  ...
  In period \period{II}, \FRA has an \terme{Artillery value} of 3 per
  \ARMY\faceplus. Thus, it has only an \terme{Artillery value} of 1 per
  ...
  \terme{Artillery value} of 2 or more) = 5 \terme{Artillery value} for all
  game purposes (battles and sieges). A stack of \ARMY\faceplus
  \ARMY\facemoins has 3 + 1 (second \ARMY with \terme{Artillery value} of only
  1) = 4 \terme{Artillery value}. Lastly, a stack of 3 \ARMY\facemoins of \FRA
  only has 1 (first \ARMY) + 1 (second \ARMY with \terme{Artillery value} of
  1) = 2 \terme{Artillery value} (\emph{i.e.}  the third \ARMY counter does
  not add anything to the \terme{Artillery value}).
\end{exemple}
...
\aparag[Naval counters.] Naval counters for \ND and \FLEET come with
different images for flavour only. A counter with a warship image can be used
for a \NGD or \FLEET containing galleys.
\bparag However, a country may never have two \FLEET counters with the same
number in play at the same time.
\bparag For example, \FRA may have up to five \FLEET counters in play at the
same time. Two of the counters have galleys image for flavour. French \FLEET
number I and II have both a counter with galleys and one with warship. \FRA
may not have both counters with name ``Fleet I'' in play at the same
time. \FRA may have five \FLEET counters full of galleys in play at the same
time (using the counters with warships image as galleys \FLEET).
...
\terme{Artillery value} for \ARMY), using \FLEET rather than \ND usually
...
\StackMorale{Mixed stacks.}

\begin{exemple}[Veteran or Conscript?]
...
\bparag The difference only occurs for Naval technologies \terme{Vessel},
\terme{Three-decker}, and [BLP] for \TUR Galleys.
...
\aparag[Size] La \textbf{taille} d'une pile militaire est donnée par la
...
\begin{exemple}[Replacing unnamed leaders]
...
\begin{exemple}[Naval moves through ports]
  It is possible, with a \TP placed in \ville{Kyoto}, to enter in the same
  move with 1\NWD coming from \seazone{Japon} and going out from
  \seazone{Philippines}, without going through \seazone{Jaune} nor
  \seazone{Pacifique NW}. However, it is not possible with a \FLEET, nor if
...
active naval action, the port is then marked with a \SUPPLY counter),
then it is considered to be non-blockaded for this round.
