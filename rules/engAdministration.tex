...
explained in \ref{chapter:ThePowers}, and serve mostly in
\ref{chapter:Incomes} and theses chapters. The nature of military troops is
explained in~\ref{chMilitary:Description of Military forces}.
...
\begin{playtip}
  ...
  resolve them two by two (one player resolves his actions with the other
...
\aparag[Administrative operations]\label{chAdministration:Administrative Actions}
...
\aparag[Administrative limits] \label{chAdministration:Administrative Limits}
Each player is entitled to a certain number of actions:
...
\begin{exemple}[Administration limits]
...
called investment: each action can be made with a \terme{Basic investment}, a
...
% described in \ruleref{chAdministration:Technology}.
\bparag The sum of all investments goes in \lignebudget{Administrative
  actions}.

\aparag Each action (except Exceptional taxes and Bankrupcy) is resolved
...
\aparag[Maximum level] The maximum level is used mainly to determine
...
% levels of commercial fleets (\ruleref{chAdministration:Minor Commercial Fleets}).
...
after they are contracted. If \lignebudget{International loans refunds} is not
empty, an international loan ends this turn. Write in \lignebudget{Mandatory
  loan refund} the amount in \lignebudget{International loans refunds}.
\bparag National loans don't need to be refunded\ldots

\begin{exemple}[Loan interests]
...
\bparag In case of collapse, the country \textbf{must} makes a \terme{Complete
  bankruptcy} (see~\ref{chAdministration:bankruptcy} below).



\subsection{Bankruptcy}\label{chAdministration:bankruptcy}
...
\bparag Erase all international loans: Write in \lignebudget{International
  bankruptcy} the amount of ongoing international loans (sums of the amounts
in \lignebudget{International loans refunds} of all the following turns) and
erase any values currently in \lignebudget{International loans interests} and
\lignebudget{International loans refunds} for the following turns.
\bparag Loss 30\VPs.
...
and diminish the \lignebudget{International loans refunds} of the following
...
\lignebudget{International loans interests} for the following turns). Only one
...
\aparag[National Loans] Choose the amount of refund you want to do. It must be
smaller than the current amount of National Loan (difference between
\lignebudget{National loans at start} and \lignebudget{National loans
  bankruptcy}).
\bparag Write this amount in \lignebudget{National loans refunds}.

\aparag[International Loans] Choose the amount of refund you want to do in
advance. It must be smaller than the current amount of International Loans
(the total of \lignebudget{International loans refunds} for all the following
turns, but not the current one).
\bparag Write this amount in \lignebudget{International loans refunds}, adding
it to the current value if any.
\bparag Decrease the values in \lignebudget{International loans refunds} by
...
\lignebudget{International loans interests} by 10\% of the \textbf{initial}
amount of that loan, for all the turns between the current one (excluded) and
the last turn of the loan (included).

\aparag Write in \lignebudget{Optional loan refunds} the sum of
\lignebudget{International loans refunds} (the part that was not taken into
account in \lignebudget{Mandatory loan refund}) and \lignebudget{National
  loans refunds}.

\begin{exemple}[Simple loan: contracting the loan]
  ...
  \lignebudget{New International loan} of turn 1 (new money), 100\ducats in
  \lignebudget{International loans refunds} of turn 4 (refund in 3 turns) and
  10\ducats in \lignebudget{International loans interests} of turns 2, 3 and
...
\begin{exemple}[Simple loan (cont.): partial refund]
  At turn 2 (during the Administrative phase), \FRA decides to refund
  40\ducats of this loan. Thus, it write 40 in \lignebudget{International
    loans refunds} of turn 2 and decrease \lignebudget{International loans
    refunds} of turn 4 by the same amount (leaving 60). Since the loan is
...
\begin{exemple}[Simple loan (cont.): term]
...
\begin{exemple}[Simple loan (cont.): complete refund]
  Suppose now that at turn 3, \FRA decides to refund the last 60\ducats of the
  loan. Thus, it writes 60 in \lignebudget{International loans refunds} of
  turn 3 and decrease \lignebudget{International loans refunds} of turn 4 by
  ...
  \lignebudget{International loans interests} for turn 4 only (all the turns
  ...
  looking back at \lignebudget{New International loan} 3 turns before the
...
\begin{exemple}[Double loan: contracting the loans]
...
\begin{exemple}[Double loan (cont.): partial refund]
  At turn 3, \POL decides to refund 60\ducats of international loans. Thus, it
  writes 60 in \lignebudget{International loans refunds} of turn 3. \POL now
  has to decrease the amount of ongoing loans (that is,
  \lignebudget{International loans refunds} of future turns) by 60. It may
  ...
  Thus, \POL sets \lignebudget{International loans refunds} of turn 5 to
  ...
  have to be removed. Thus, \POL removes 6 from \lignebudget{International
    loans interests} of turns 4 and 5 (between now and the term of the
...
\section{Domestic operations}\label{chAdministration:Domestic}
...
\aparag[Manufacture placement]\label{chAdministration:Manufacture Placement} The \MNU
...
\begin{exemple}[Developing \MNU (continued)]
...
\aparag[Other Trade Indexes]\label{chAdministration:Special FTI} Some countries
...
\bparag At least one owned national province is either controlled or besieged
by an enemy in this war.
...
\aparag \textbf{Remember:} do not roll for exceptional taxes during
...
\begin{exemple}[Good taxes]
  ...
  Prussian \STAB to 0. \monarque{Friedrich II} decides to raise exceptional
...
\begin{exemple}[Bad taxes]
  ...
  The French king, \monarque{Henri III} has an \ADM of 6. and \FRA has a \STAB
...
% following modifiers:
\bparag See the access limitations in \ruleref{chAdministration:Limited
  Access} for restriction on the seas that may be targeted.
...
\item[-1]If at least one \pays{pirates} \corsaire is present in the \STZ/\CTZ.
...
levels.
\bparag See \ruleref{chAdministration:Trade Competition Mandatory} if two
fleets are \Faceplus in the same \STZ/\CTZ or when one \TradeFLEET reaches
level 6.
...
\begin{exemple}[\TradeFLEET implementation]
...
in the \ROTW that does not already contains a foreign \COL, a \terme{Minor
  establishment} or a \terme{Pirate haven}. Then use column
...
\ref{chAdministration:Inland advance} and \ref{chAdministration:Native
  empires} for restriction on the provinces that may be targeted.
...
\aparag\label{chAdministration:Colony:Critical failure} An unmodified result
of 1 or 2 (even if the action is a success) requires a second roll of 1d10: if
it is \emph{strictly less} than the Tolerance value for the \Area, the natives
are immediately activated and will attack during the redeployment phase (as
per \ruleref{chRedep:Native Attack}).
...
\begin{exemple}[Developing \COL]
  ...
  On turn 2, \HIS has left \leaderwithdata{Colon} in \granderegionCuba and
...
\aparag[Critical failure]\label{chAdministration:TP:Critical failure} A result
...
own either the \CCs{Grand Orient}, the \CCs{Mediterrannee}, or a province (or
...
\bparag \rulelabel{chAdministration:Limited Access:Giving Rights} The country has
...
is no more in \MA.
\bparag The \STZ is either \stz{Canarias}, \stz{Guinee}, \stz{Arabie},
\stz{Indien} or \stz{Chine} and the country has discovered all the sea zones
of the \STZ (i.e. the sea zone in which is the symbol, plus all neighboring
ones).
%% [TBD: Aiguilles exclu car pas de civilisation construite et commercante ici]
...
\begin{exemple}[Access to \stz{Canarias}]
  ...
  competition targeting any other \TradeFLEET in \stz{Canarias}.
\end{exemple}
...
\bparag \SUE ignores this restriction if it has a policy of \terme{Overseas
  expansion}.
...
\begin{modlist}
\item The province is coastal and linked by a continuous path of known sea
  zone to a \COL, \TP or European province owned by the country.
\item[OR] The province is within supply distance (12\MP) by land only of a
  \COL, \TP or European province owned by the country; this path may not go
  through provinces belonging to a minor, or with any kind of establishment
  not belonging to the country (troops do not hamper it)
\end{modlist}
...
\section{Other administrative operations}\label{chAdministration:Techno and Competition}
...
\begin{exemple}[Technological advancement: bonus box]
...
\begin{exemple}[Technology: Cultural group adjustment]
...
\begin{exemple}[Technological goals: Time adjustment]
...
\begin{exemple}[Technological goals: Blocked by another goal]
...
\subsubsection{Technology improvement}\label{chAdministration:Technology
  Improvement}
...
\begin{designnote}
  The technological track does not bounds the technological level a country
  may reach. In the rare case where a country reaches a technological level
  above 70, use any mean you want to record its technological level (we advise
  buying another copy of the game so you can have a second technological track
  and put stickers on it with the correct numbers).
\end{designnote}
...
\begin{exemple}[Spreading technological revolutions]
...
\subsection{Competitions}\label{chAdministration:Competition}
...
\begin{exemple}[Freed exotic resources]
  ...
  \paysGujerat) - 1 (Investment of \paysGujerat) = 3. There is a \bonus{-1}
...
\begin{exemple}[Mandatory competition of \TradeFLEET]
...
\aparag[Order of competitions] If the player disagree on the order in which
...
(\ruleref{chAdministration:Speculation}).% If not doing \terme{Speculation}, all
...
\begin{exemple}[Automatic competition for exotic resources]
  \HOL, \ENG and \FRA are competing for the exploitation of two \RES{Spices}
  resources. \HOL and \FRA both have enough levels to exploit one and \ANG can
  exploit both. \ANG and \HOL have \TP while \FRA has a \COL.
...
\bparag This action must target one \CTZ/\STZ where the \terme{maximum level}
is strictly less than the \terme{reference level}.
