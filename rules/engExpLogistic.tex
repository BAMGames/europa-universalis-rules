...
technology \TARQ (representation of the popularisation of the \emph{Trace
  Italienne}).
\bparag Maintenance is doubled for fortresses of level 4 before Turn 40
(representation of the new ideas of Vauban).
...
\bparag The level of a fortress can be lowered by its owner at will (unless
some specific rules prevent it). A lowered fort is destroyed.
...
\bparag A fortress may only be built in an owned, controlled, not besieged and
not revolted province.
...
\ruleref{chPeace:BuildPresidios}, during the redeployment phase of the
...
limit.
\bparag \NGD and \NDE count as half a \ND for this limit. \VGD and \NTD count
as a full \ND.
...
\bparag It is also summarised in~\ref{table:Recruitment per Country} (second
line, second number).
\bparag \NGD and \NDE count as half a \ND toward this limit. \VGD and \NTD
count as a full \ND.
...
\aparag After all Administrative and Logistic actions by \MAJ have been
resolved, Reinforcements can be obtained for each \MIN that is fully at war,
according to one of the following 3 attitudes: Offensive, Defensive or Naval.
\aparag An attitude is chosen at the end of the Logistics phase and is valid
for the current turn only. The choice may be changed every applicable turn.
In some cases, events impose the choice of the attitude.
\aparag
Once an attitude is chosen, one die is rolled and the Reinforcements Table
...
\aparag This attitude has an impact on forces that the minor is going to
obtain and on the way to conduct the war, namely:
\aparag[Offensive:] the minor country has to move at least a stack into enemy
...
\aparag[Defensive:] the minor country cannot move any stack outside of its
territory, adjacent provinces and ex-territory.
\aparag[Naval:] the minor must move at least one naval stack to undertake an
...
territory.  The naval attitude may only be chosen for a minor country that has
naval counters at its disposal.
\aparag The choice of the attitude is left to the player that controls the
minor country, excepted when an events dictates otherwise. For instance, if
the minor is the attacker (as described in the event), the player must usually
choose an Offensive attitude (or Naval if the minor country has fleet or naval
detachments counters), as per event description. This choice may be changed
each turn.
...
\aparag According to which attitude is chosen, one die is rolled and the
Reinforcements Table indicates the result. Modifiers are applied to the die.
Each column tells the number of \LD, fortresses, \ND or \FLEET and Muliple
Campaigns (CM) obtained. These are deployed in the minor territory.

\aparag[Modifier to Reinforcements]
Some modifiers may be applied to the die-roll for reinforcements. All
modifiers are cumulative.

\bparag There is a Political modifier specific to some minor countries.  Such
a modifier is indicated on the Reinforcements Table, and also in the country's
description in the Annexes.

\bparag The reinforcement die-roll is also modified cumulatively by a modifier
linked to the minor country income. That income is calculated on the basis of
provinces owned and controlled by the minor at the time the reinforcement
die-roll is made.

\aparag[Military Leaders of Minors]
Some minors have military leaders in their \terme{basic forces}.  This is
indicated in the Annexes. In addition the Reinforcements Table gives the
possibility of additional military leader(s).
\bparag A military leader is obtained by the roll of a D10 equal to or
inferior to the value indicated on the Reinforcements Table under the rubric
Leaders.
\bparag A General is then picked at random among gray-colored minor (?)
leaders still available.
\bparag In case a naval attitude has been chosen for the minor and a leader is
obtained, an (?) Admiral can be picked up in place of a General.

\aparag[Specific Cases of \terme{Veteran} Reinforcements]
\bparag All land reinforcements are conscripts, except for the following \MIN:
\bparag Swedish new forces obtained are \terme{Veteran};
\bparag Swiss new forces obtained are \terme{Veteran} if their Land Technology
is less than \technologie{Baroque};
\bparag Persia receives half of its new forces as \terme{Veteran} (round
down).

\aparag[Fortresses]
Some levels of fortresses are obtained by the Reinforcement Tables.
\bparag To that, from period V on, add one level of fortress to the basic
forces of every minor country having at least 16 of Income.
\bparag A given fortress can not be improved by Reinforcements by more than
one level at a given turn. This rule does not contrain the fortresses that are
in the Basic Forces.
\bparag Before turn 40, the level 4 can only be placed by minor countries at
the cost of 2 levels of fortress on top of an existing level 3.
...
\section{Exotic resources price variation}
...
\continent{Bresil} and elsewhere is still considered to be the same resource.
...
The economic situation die roll gives an \terme{economic situation}
...
\bparag If he has either a total or partial monopoly (see
\ruleref{chIncomes:MonopolyExoticResources}), he puts the production counter
upside-down. The resource is considered to have a \terme{rare} production
level (green) for the computation of its basic variation.
...
\begin{exemple}[Prices of exotic resources]
  ...
  rare production. Adding a die-roll of 3 yields a result of 5, smaller than
  the low threshold, thus giving a market variation of +1.  The total
  variation is thus +3, which is capped at +2. The price marker should be
  moved 2 boxes to the right. However, the maximum price for \RES{Salt} is the
  leftmost 7 box, so the marker stops there.
\end{exemple}



\subsection{Attribution of centres of trade and convoys}
...
is credited only upon arrival in a port during the military phase and the
redeployment, to the owner of the port (or its Patron, if it is a minor
vassal).
\bparag The sum is reported in \lignebudget{Gold from ROTW &
  Convoys}.%, but does not count for
% \ruleref{chPeace:InflationGoldROTW} and \ruleref{chPeace:InflationGold} in
...
convoy %is a fleet of
% transport,
contains 3 \NTD, each carrying an income of 15\ducats.  It appears every turn
in \province{Izmir}, at the administrative phase, if the \CCs{Grand Orient} is
owned by \TUR.
...
vassal) or controls the \CCs{Mediterranee}, and it accepts the convoy.
\bparag If \TUR attributes the convoy to another player, it receives 20\ducats
as soon as the convoy reaches the other player.
\bparag The Turkish player may refuse to grant the convoy or possibly no one
accepts it, and, in such a case, immediately loses 20\ducats from the revenue
of the \CCs{Grand Orient} and 1\STAB.
...
least 10 of them.
\bparag In case of ties, the convoy is attributed to the player controlling
the \emph{Atlantic} Trade Centre if he is among the ties, to the Netherlands
before 1700 (turn 43) and to England from 1700 onward. If there are still
ties, it is not attributed.
\bparag The East Indies convoy must start from a \COL\faceplus or a
\TP\faceplus in \continent{Asia}, and must reach a European port of its
owner. It is worth 4 \NTD carrying 15\ducats each.
\bparag The gold obtained when reaching a continental port with a convoy is
accounted for in \lignebudget{Gold from ROTW & Convoys}, even if the convoy
was seized from another player.
...
fleet can only transport gold stored in Spanish ports. The \terme{Flota de
  Oro} is equivalent to 5 \NTD, as well as the \terme{Flota del Per\'u}.
...
\section{Deployment [to be moved in ch. VI]}
