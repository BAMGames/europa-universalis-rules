...
\definechapterbackground{Logistic}{ColletRecruitingSergeant}
...
\construction{Malta}, \construction{Saint-Petersbourg} and
\construction{Sebastopol}, arsenals may only be built in a coastal \TP or
\COL (including \COL of level 6).
\bparag The named arsenal \construction{Brazilie} can only be built in
\continent{Brazil}.
\bparag The named arsenal \construction{Overseas} can only be built if \SUE
has a policy of Overseas expansion.
\bparag The named arsenal \construction{Gibraltar} can only be built in
\province{Gibraltar} or \province{Tanger}.
\bparag The named arsenal \construction{Malta} can only be built in
\provinceMalta, and only if this province is owned by \TUR.
...
\begin{exemple}[Naval recruitment in the \ROTW]
...
\begin{exemple}[Minors basic forces]
  ...
  as usual). That is, the country needs one turn of peace to recruit its
  troops up to its basic forces.
\end{exemple}
...
\begin{exemple}[Removing named leaders]
  ...
  he is considered as having turns 26-29 on his counter. At the Administrative
  phase of turn 30, if he is still alive, \leaderTilly is removed from game
...
\LeaderGov) where a country has less leaders than its minimum, draw as many
...
\begin{exemple}[Replacing a \LeaderC]
  ...
  \LeaderE, \leaderSolis may be placed exactly where \leaderColon
  was.
