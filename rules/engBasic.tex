...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      ...
      \image(6,0){Leader_turcorsaire_Siroco}
      \legendesimple{Leader_turcorsaire_Siroco}{1.3}{\textbf{Privateer}}
      \legendelongue{Leader_turcorsaire_Siroco}{-.9}{Mediterranean only}
      \pointe(.55,-.15){Leader_turcorsaire_Siroco}
      \legendesimple{Leader_turcorsaire_Siroco}{.9}{Name}
      \pointe(0,.5){Leader_turcorsaire_Siroco} \image(8,0){Leader_usa_Arnold}
      ...
      \image(8,-3.2){Pacha_timar_Ali_recto}
      \legendesimple{Pacha_timar_Ali_recto}{.9}{Normal}
      \image(10,-3.2){Pacha_timar_Ali_verso}
      \legendesimple{Pacha_timar_Ali_verso}{.9}{Corrupted}
      \legendedouble{Pacha_timar_Ali}{1.3}{\Pasha counter}
      \legendesimple{Pacha_timar_Ali_verso}{-.9}{Pasha id}
      \pointe(-.45,.25){Pacha_timar_Ali_recto}
      \pointe(-.45,.25){Pacha_timar_Ali_verso}
      \legendesimple{Pacha_timar_Ali_recto}{-.9}{Force (\LD)}
      \pointe(.55,-.5){Pacha_timar_Ali_recto}
    \end{tikzpicture}
  \end{center}
...
\begin{deflist}
  ...
  \listingabbrev{DTI}{Domestic Trade Index, a value that measures the domestic
    commercial power of a major power}
  \listingabbrev{EcoRS}{Economic Record Sheet}%, displayed on
%    page~\pageref{economic-record-sheet}}
...
\subsection{Continents and Sea regions in the \ROTW}
\begin{designnote}
...
\item[\anchorregion{Atlantic}] contains all the \ROTW sea zones that can be
  reached (by sea) from \seazone{Atlantique} without \emph{exiting} a zone
  with malus, including \seazoneHorn.
...
\item[\anchorregion{Indian}] contains \seazone{Quarantiemes} plus all the sea
  zones of the \stz{Indien}, \stz{Arabie} and \stz{Aiguilles}, except
  \seazoneTempetes.
...
\item[\anchorregion{Pacific}] contains all the sea zones of the \ROTW that are
  neither in \regionAtlantic nor in \regionIndian, plus \seazoneHorn (which
  belong both to \regionAtlantic and \regionPacific).
...
\item[\anchorregion{Mediterranee}] contains all the sea zones of the
  ...
  \seazoneMarmara, \seazoneEgee, \seazoneLevantin,
  \seazoneAdriatique, \seazoneIonienne, \seazoneThyrrenienne,
  \seazoneGabes, \seazoneLion, \seazoneAlger, \seazoneAlboran). % Galleys may
  % navigate it.
