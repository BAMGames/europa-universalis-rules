...
\aparag[New \Pashas.] \TUR has a maximal number of \Pashas in play
...
\aparag[Corruption Cost of \Pashas.]
...
are not. The \terme{Sole Defender of the Catholic Faith} places each of
...
\aparag[Death of a \Pasha] [BLP] This effect occurs on a modified roll
of 5 or 7 at the survival test.
\bparag The \terme{Sole Defender of the Catholic Faith} may choose one
...
one and replace it.
\bparag The \terme{Sole Defender of the Catholic Faith} and \TUR take
turns replacing \Pashas that way until one of them decides to stop.

\aparag[Dismissal of \Pashas.] The Turkish player can dismiss (or
...
\aparag[\Pashas and Hierarchy] Even if \Pashas can be used as generals,
...
\aparag[\Pashas troops] The force content in \LD of a \Pasha cannot be
...
\bparag A test is made on \tableref{table:Administrative Actions},
...
[BLP] Add 2\LeaderG to the limit for \TUR.
...
\aparag[Barbary Coast privateers] Privateers \LeaderA (not privateer
...
\listleadershort{leadersturcorsaire}. They have a red symbol instead
of a black one.
...
country.
\aparag[Relations with the Knights.]
The \pays{Chevaliers} are in permanent semi-Overseas war against
...
\aparag[Crusades and Turkish occupation of Vienna]
See rules \ruleref{chSpecific:Crusades and Vienna}.


\subsubsection{Relations with the \terme{Barbaresques}}
\aparag Depending on several events, \TUR may have geopolitical malus to
all diplomacy attempts against all \terme{Barbaresque} countries.
\bparag Initially (before event \eventrefname{pII:Alignment of
  Barbaresques}), \TUR has a {\bf -3} malus to all diplomacy attempts
against all \terme{Barbaresque} countries.
...
From period III to period IV, if \TUR has a \TP in \continent{Asia}
...
go in \continent{Asia}.

\aparag[\leader{Selman Reis}] [BLP] After the fall of \paysMamelouks,
\leader{Selman Reis} serves \TUR.
...
%\aparag[The \Presidio at Istanbul]\label{chSpecific:Turkey:Dardanelles} The
%\province{Trakya} province, as long as it belongs to \TUR, can be used
%as a \Presidio of level 2 on any fleet that tries to enter or exit
%\seazone{Noire W} for \seazone{Marmara}.

% (Jym)
% Devrait maintenant être dans l'event...
%\subsubsection{Modification event \sectionministre{Koprulu}}
%\aparag Event shall be put as early as period IV.
% toujours en pV.

%\aparag Event may not take place if reforms of a level higher than 1
%were successful.
%\aparag The dynasty of viziers lasts automatically 8 turns (may be
%dismissed before) [1656-1691].
%\aparag Reforms are impossible during the duration of the event.
%\aparag \leaderwithdata{Koprulu} is bound to \eventref{pV:Koprulu}
%naming a member of this dynasty to the distinction of Grand Vizier.
% le reste est OK.
...
\bparag[Troubled succession] [BLP] He has \terme{Fragile health}
...
\aparag[\anchorministre{Sadrazam}] [BLP] There are two kind of Turkish
counters with the monarch symbol.
\bparag The \terme{Sultan} are \leaderSuleyman, \leaderSelim and the
generic Sultan counter.
\bparag The \terme{Vizier} are all the other. They have a golden crown
symbol instead of a black one.
\bparag List of Viziers: \listleadershort{leadersturvizir}.
% , namely \leaderSokollu and
% the generic Sadrazam counter.
...
% counter replaces usual Sadrazam.
...
% LocalWords: Turquie Mediterranee malus feudality Trakya
