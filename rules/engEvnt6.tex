...
\section{Period VI}\label{events:pVI}
...
\bparag \paysecosse is not considered to be \VASSAL or \ANNEXION by \ANG as
long as the war lasts (for incomes or victory condition purpose).
\aparag Place a \REVOLT \faceplus in each of the following provinces:
\provinceHighlands, \provinceMoray and \provinceAlba.
\bparag Place a \ARMY\facemoins of \paysecosse and general \leader{Prince
...
\aparag If \ENG is at war with \paysportugal allied to a \MAJ, \paysportugal
...
\bparag If \ENG is at war with \paysportugal (not allied to a \MAJ), then the
event cannot occur. Mark-off and play \RD instead.
...
\phevnt
\aparag Natives of area \granderegionNatal and the two provinces bordering it
are activated for this turn and shall attack all \COL/\TP in these provinces.
...
\aparag \TUR has a malus of \bonus{-3} to diplomacy with all
\terme{Barbaresques} countries (\paysCyrenaique, \paysTripoli, \paysTunisie,
