...
\section{Period IV}\label{events:pIV}
...
\RUS.
\bparag The military forces of the Revolt of Gabor can use up to 1 \ARMY and 2
\LD of the Hungarian counters (and the \HAB can use at most one \ARMY and 2
...
\aparag The Revolt of Gabor consists of one Hungarian \ARMY \faceplus
(Conscripts) and \leaderwithdata{Gabor} (lasting 4 turns) placed in the
province of the \REVOLT .
\aparag None of \paysBaviere, \paysBoheme, \paysPalatinat and the Revolt of
Gabor receive reinforcements on the first turn. They receive normal
reinforcements beginning with the second turn of the war.
\aparag The reinforcements of the Revolt of Gabor are based only on the
provinces in \payshongrie that he controls or that are in \REVOLT . If there
are none, or if \leader{Gabor} is not in play (dead or wounded), it receives
no reinforcements.
...
with smaller forces.
\aparag[Gabor]
\bparag The forces of \leaderGabor are always in restricted supply in the
...
\bparag A force lead by \leaderGabor can withdraw in \paysTransylvanie or
...
any following turn.
\bparag As long as \leaderGabor and/or its forces are in \paysTransylvanie or
national provinces of \payshongrie owned by \TUR, \TUR has a free \CB against
\leaderGabor. If \TUR uses it, the forces of \leaderGabor are also at war
against \TUR and \TUR may enter, besiege and attack any force in national
provinces of \payshongrie.
...
\aparag The \royal receive the general \leaderwithdata{Rupert} on the first
...
(\textit{The New Model Army}, see rules \ruleref{chExpenses:Military
...
\aparag Rebels have the general \leaderDimitri (until he dies) for 5 turns.
...
\leaderDimitri also rolls for survival with a \bonus{+3} as sole modifier
during the event.
...
has a \CB to do so, or a free \CB is \leaderDimitri is alive. This
...
if \monarque{Boris Godunov} is dead and Loyalists surrender willingfully.
...
\aparag If the Loyalists win, \leaderDimitri is eliminated.
...
\bparag If \leaderDimitri is alive, he becomes tsar \monarqueDimitri with
...
\bparag If \leaderDimitri is dead, the new ruler is \monarqueRomanov for 5
...
% minor is war to the major, so no way no to help
...
% LocalWords: Vassalisation Gabor Mansfeld Westphalie Ausgsburg Schmalkaldic
