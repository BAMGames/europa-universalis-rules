...
\xnameref{pIV:Union Scotland} is effective. The Royalists control the fortress
...
\bparag \xnameref{pVI:Act Union} is broken or, if it did not happen yet, will
not occur later.
...
\bparag[Mediterranean Concessions] \provinceGibraltar and \provinceBaleares.
...
\condition{If \monarque{Peter the Great} was already received, nothing happens
  with this event.}
...
\phevnt
\aparag The king of \POL dies. The new king is \monarque{August II}, elector
of Saxony.
...
royal crown.
% (Jym) Does it mean that if the royal dignity is not given, let's say, before
% WoAS, PRU may not hope to gain anything but the crown (especially not
% \provinceSilesie/\provinceLausitz? This looks a bit rough especially since
% WoAS is may be the first time that \PRU is played, so before the player may
% really have an influence on the crown (typically, if \AUS does not need the
% alliance of \PRUmin in WoSS)...
