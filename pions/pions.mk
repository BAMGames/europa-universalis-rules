...
$(PIONSDIR)/depends: $(PIONSDIR)/pions.pdf
	...
	@sed -ne '1,/^# Automatic section/ p' < $(PIONSDIR)/pions.mk > $(PIONSDIR)/pions.mk2;grep ^\* $(PIONSDIR)/depends.txt |cut -f2- -d/|grep ^blasons|grep -v pgm|cut -f2 -d_|cut -f1 -d.|LC_ALL=C sort|xargs echo "PIONSBLASONS =" >> $(PIONSDIR)/pions.mk2;grep ^\* $(PIONSDIR)/depends.txt |cut -f2- -d/|grep ^shadow|cut -f2 -d/|cut -f1 -d.|LC_ALL=C sort|uniq|xargs echo "PIONSSHADOWS ="  >> $(PIONSDIR)/pions.mk2; sed -ne '/^# End automatic/,$$ p' < $(PIONSDIR)/pions.mk >> $(PIONSDIR)/pions.mk2; mv $(PIONSDIR)/pions.mk2 $(PIONSDIR)/pions.mk
